...
class Gatt(object):
        ...
        for serv_desc in gatt_desc:
            ...
            if "chars" in serv_desc:
                for char_desc in serv_desc["chars"]:
                    ...
                    new_characteristic = new_service.add_characteristic(
                        char_desc["name"], char_desc["uuid"], char_form
                    )

                    if "descriptors" in char_desc:
                        for desc_desc in char_desc["descriptors"]:
                            desc_form = (
                                desc_desc["fmt"] if "fmt" in desc_desc else FormatRaw
                            )
                            _ = new_characteristic.add_descriptor(
                                desc_desc["name"], desc_desc["uuid"], desc_form
                            )
    ...
    def resolve(self, resolve_timeout=10, warn_unmatched=True, resolve_unknown=True):
        ...
        if not self.dev.services_resolved:
            self.logger.debug(
                "Services not resolved, waiting for it for max: {}s".format(
                    resolve_timeout
                )
            )
            if resolve_timeout > 0:
                if not self.dev.wait_services_resolved(resolve_timeout):
                    raise bzerror.BluezFailedError(
                        "Timeout waiting for services resolved"
                    )
            else:
    ...
    def help_keys(self):
        ...
        for s in self.services:
            for c in s.chars:
                print(
                    ".{}.{} ({}found)".format(
                        _make_id(s.name), _make_id(c.name), "not " if not c.obj else ""
                    ),
                    file=sys.stderr,
                )
...
class GattService(BluezInterfaceObject):
    ...
    def __str__(self):
        return "{}(obj='{}',name='{}',uuid='{}')".format(
            self.__class__.__name__.split(".")[-1], self.obj, self.name, self.uuid
        )
    ...
    def _resolve_characteristics(
        self, objects, warn_unmatched=False, resolve_unknown=True
    ):
        """
    ...
    def __getattr__(self, name):
        ...
        raise AttributeError(
            "No member '{}' contained in GattService object.".format(name)
        )
...
class GattCharacteristic(BluezInterfaceObject):
    ...
    def read(self, options=None, raw=False, native=True):
        ...
        if self._proxy:
            try:
                v = bzerror.callBluezFunction(
                    self._proxy.ReadValue, options, timeout=to
                )
    ...
    @bzerror.convertBluezError
    def read_async(
        self, success_cb, error_cb, user_data, options={}, raw=False, native=True
    ):
                ...
                def _success_cb(proxy, result, data):
                    ...
                    if len(result):
                        ...
                        try:
                            ...
                            err = bzerror.BluezFormatDecodeError(
                                "{}: {}, got: {}".format(self, str(e), str(value))
                            )
                        ...
                        error_cb(
                            self,
                            bzerror.BluezFailedError("No value was returned"),
                            data,
                        )
    ...
    def onValueChanged(self, func, *args, **kwargs):
        ...
        if self.obj:
            ...
            @wraps(func)
            def valueChangedCallback(
                gatt_char_self, changed_values, *cbargs, **cbkwargs
            ):
                if "Value" in changed_values:
                    gatt_value_obj = gatt_char_self.fmt.decode(changed_values["Value"])
                    func(gatt_char_self, gatt_value_obj, *cbargs, **cbkwargs)

            self.onPropertiesChanged(
                valueChangedCallback, *args, prop="Value", **kwargs
            )
        else:
    ...
    def notifyOn(self):
        if self.obj:
            ...
            raise bzerror.BluezDoesNotExistError(
                "Failed to enable notify: {}".format(str(self))
            )
    ...
    def __getattr__(self, name):
        """ will only get called for undefined attributes """
        raise AttributeError(
            "No member '{}' contained in GattCharacteristic object.".format(name)
        )
