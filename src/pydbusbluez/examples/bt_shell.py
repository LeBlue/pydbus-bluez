...
def main():
    ...
    parser.add_argument(
        ...
        nargs="?",
        default=None,
        help="device address(es) to connect to",
    )
    ...
    parser.add_argument(
        ...
        metavar="MOD",
        default=None,
        help="gatt description to import (PACKAGE.MODULE:[GATT_VARIABLE]",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        type=str,
        help="commands to run from script(s), see the run/record commands",
    )
...
class BTShell(cmd.Cmd):
    ...
    def do_set(self, arg):
        ...
        if len(args) < 2:
            print(
                'set: Need an argument, simple python types, e.g 1, "foo", (1, 5), [0,4,5]',
                file=sys.stderr,
            )
        ...
        if not issubclass(fmt, FormatBase):
            ...
            if issubclass(fmt, FormatTuple):
                ...
                for idx, sc in enumerate(fmt.sub_cls):
                    try:
                        fields.append(
                            "'{}': {}".format(fmt.sub_cls_names[idx], sc.__name__)
                        )
                ...
                print(
                    "{} ({})".format(
                        str(fmt.__name__), ", ".join(fields), file=sys.stderr
                    )
                )
            else:
                print(
                    "{} python native: {}".format(
                        str(fmt.__name__), str(fmt.native_types), file=sys.stderr
                    )
                )
    ...
    def do_connect(self, arg):
        ...
        if (
            _gatt_valid(self.gatt)
            and self.gatt.dev.name.lower() == args[0].lower()
            and self.gatt.dev.connected
        ):
            print("Already connected", file=sys.stderr)
                    ...
                    for c in sorted(s.chars, key=lambda x: x.uuid):
                        ...
                        if c.obj:
                            ...
                            if len(c.descriptors) > 0:
                                schema_file.writelines(['\t\t\t\t"descriptors": [\n'])
    ...
    def do_notify(self, arg):
        ...
        try:
            if timeout_seconds:
                print(
                    "Notifying for {} seconds".format(timeout_seconds), file=sys.stderr
                )
            else:
